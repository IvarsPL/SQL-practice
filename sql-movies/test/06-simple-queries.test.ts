...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `select Count(*) as count from movies where budget > 100000000 and release_date >= date(2009-01-01)`;
      const result = await db.selectSingleRow(query);
  ...
  it(
    ...
    async done => {
      const query = `select original_title, budget, revenue from movies where release_date >= date('2009-01-01') order by budget desc limit 3`;
  ...
  it(
    ...
    async done => {
      const query = `select count(*) as count from movies where substr(homepage, 0, 6) = 'https'`;
      const result = await db.selectSingleRow(query);
  ...
  it(
    ...
    async done => {
      const query = `select count(*) as count, substr(release_date, 0, 5) as year from movies group by year order by year desc`;
      const result = await db.selectMultipleRows(query);
